...
response.sendRedirect("login.jsp");
  ...
  try
  {
   rs = con.createStatement().executeQuery("SELECT count(*) from fitservice.nationaltv_pgm_Users where username=N'" + username.replaceAll("'", "''") + "' and password=N'" + password.replaceAll("'", "''") + "'");             
   userexit= 0;
   while(rs.next()){
...
//String progid = "";
//rs1 = con.createStatement().executeQuery("SELECT * from fitservice.nationaltv_pgm_Users where username=N'" + username.replaceAll("'", "''") + "' and password=N'" + password.replaceAll("'", "''") + "'");

session.setAttribute("username",username);
session.setAttribute("password",password);

//session.setAttribute("", arg1)
con.createStatement().execute("update fitservice.nationaltv_pgm_Users set   lastlogintime= NOW() where username=N'" + username.replaceAll("'", "''") + "' and password=N'" + password.replaceAll("'", "''") + "'");             
...
<title>ÏÎÜÜæá ... National TV</title>
</head>
