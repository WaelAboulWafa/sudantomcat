...
response.sendRedirect("login.jsp");
  ...
  try
  {
   rs = con.createStatement().executeQuery("SELECT count(*) from report.rep_user where username=N'" + username.replaceAll("'", "''") + "' and password=N'" + password.replaceAll("'", "''") + "'");             
   userexit= 0;
...
{
String serv = "";
rs = con.createStatement().executeQuery("SELECT username from report.rep_user where username='"+ username.replaceAll("'", "''") +"';");
while(rs.next()){
	serv = rs.getString(1);
}
...
con.createStatement().execute("update report.rep_user set   last_login= NOW() where username=N'" + username.replaceAll("'", "''") + "' and password=N'" + password.replaceAll("'", "''") + "'");             
response.sendRedirect("miniutesreport.jsp");    
}
else
{
session.invalidate();
