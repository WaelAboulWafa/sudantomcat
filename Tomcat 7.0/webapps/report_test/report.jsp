...
////////////////////////// End Of Dates ///////////////////////////
	...
	rs = null;
String queryservdetails = "";
	queryservdetails = "SELECT dst, operator ";
	...
	String detailQuery = "SELECT DATE_FORMAT(call_start_date,'%Y-%m-%d') as theday , operator, app_name,dst,count(*) as thecnt , sum(CEIL(TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) / 60) ) as Minutes"; 
...
// ------------------
    ...
    String totalsquery = "SELECT  DATE_FORMAT(call_start_date,'%Y-%m-%d') as theday , count(*) ,sum(CEIL(TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) / 60) )";
...
<div align='center' style="width:30%; margin-left:40%" align="center" id="div-1a">
	...
	<table class="pure-table" border='1' align='center' bgcolor=#f4f4f4 cellpadding='0' cellspacing='0' style='border-collapse: collapse' bordercolor='#111111' width='100%'>
		...
		while(rs1.next()){
			...
			srvowner = rs1.getString(3);
			shortcode = rs1.getString(4);
			hits = rs1.getInt(5);
			minutes = rs1.getInt(6);
			%>
		...
		while(rs1.next()){
			...
			totalCalls = rs1.getInt(2);
			totalMinutes = rs1.getInt(3);
			
		}
		
		%>
		<tr bgcolor=#5CE65C >
			<td colspan="2"><font size='2'  color='000099'><font size='5' color=#FFFFFF>Total</font></td>
			<td ><font size='2'  color='000099'><font size='4' color=#0000FF><%=totalCalls %></font></td><td><font size='4' color=#0000FF><%=totalMinutes %></font></td>
		</tr>
	</table>
