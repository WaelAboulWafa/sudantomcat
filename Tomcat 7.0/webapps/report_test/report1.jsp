...
////////////////////////// End Of Dates ///////////////////////////
	...
	// the shortcodes
		...
		String servicesc = "select shortcode FROM rep_service where servid = '"+service_id+"' ";
			   //servicesc = servicesc + "FROM rep_service";
			   //servicesc = servicesc + "WHERE servid = '"+service_id+"'";
 ...
 ////////////// getting Dashboard stuff ////////////////
  ...
  rs1 = null;
  int Zain = 1;
  rs1 = con1.createStatement().executeQuery("select sum(CEIL(TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) / 60) ) as minutes from cdr  where dst  in ("+listOfShortCodes+") and operator = 'Zain' and call_start_date between '"+month_start+"' and '"+today+"';");
  while(rs1.next()){
	  Zain = rs1.getInt(1); 
  }
  rs1 = null;
  ...
  int MTN = 1;
  rs1 = con1.createStatement().executeQuery("select sum(CEIL(TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) / 60) ) as minutes from cdr  where dst  in ("+listOfShortCodes+") and operator = 'MTN' and call_start_date between '"+month_start+"' and '"+today+"';");
  while(rs1.next()){
	  MTN = rs1.getInt(1); 
  }
  rs1 = null;
  ...
  int SudaTel = 1;
  rs1 = con1.createStatement().executeQuery("select sum(CEIL(TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) / 60) ) as minutes from cdr  where dst  in ("+listOfShortCodes+") and operator = 'SudaTel' and call_start_date between '"+month_start+"' and '"+today+"';");
  while(rs1.next()){
  ...
  int Total = 1;
  rs1 = con1.createStatement().executeQuery("select sum(CEIL(TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) / 60) ) as minutes from cdr  where call_start_date between '"+month_start+"' and '"+today+"';");
  while(rs1.next()){
	  Total = rs1.getInt(1); 
  }
  rs1 = null;
...
<div align='center' style="width:30%; margin-left:40%" align="center" id="div-1a">
	...
	<table class="pure-table" border='1' align='center' bgcolor=#f4f4f4 cellpadding='0' cellspacing='0' style='border-collapse: collapse' bordercolor='#111111' width='100%'>
		<tr height=20 bgcolor=#1975FF>
                        <td colspan=5 align='center'><font size='2' color="white"><b>Per Shortcode/Operator</b></font></td>
        </tr>
		<tr bgcolor=#4791FF>
		...
		%>
		<tr bgcolor=#5CE65C >
			<td colspan="3" ><font size='5' color=#FFFFFF>Total</font></td><td><font size='4' color=#0000FF><%=totalCalls %></font></td><td><font size='4' color=#0000FF><%=totalMinutes %></font></td>
		</tr>
	</table>
