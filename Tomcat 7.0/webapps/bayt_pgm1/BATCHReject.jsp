...
for (int i =0;i< pagesize ;i ++)
{
  ...
  {
     try{
        con.createStatement().execute("update CDRs set  FilerationAgent=N'" + session.getAttribute("username").toString().replaceAll("'", "''") + "', status=-1, Update_Time=getdate() , FiltrationIP= '" + request.getRemoteAddr() + "'  where id=" + id[i]);        
