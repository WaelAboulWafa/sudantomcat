...
String query3 = "";
query3 = query3 + " SELECT ID, DATE_FORMAT(call_start_date,'%Y-%m-%d %H:%i:%S') as callstart, DATE_FORMAT(call_end_date,'%Y-%m-%d %H:%i:%S')  as callend, operator , dst ,src, TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) as Seconds , case 	when  ( TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) < 1 ) then 0 	else CEIL(TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) / 60) end as BilledMinutes ";
