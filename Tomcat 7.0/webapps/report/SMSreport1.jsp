...
////////////////////////// End Of Dates ///////////////////////////
	...
	String detailQuery = "SELECT operator, app_name,dst,count(*) as thecnt , sum(CEIL(TIMESTAMPDIFF(SECOND, call_start_date, call_end_date) / 60) ) as Minutes"; 
		   ...
		   detailQuery = detailQuery + " and operator in ('SudaTel' , 'MTN' ) and dst in ("+listOfShortCodes+")";//app_name = '"+theService+"'";
	...
	//////
String query1 = "SELECT count(*),sms.providers.name ";
...
query1 = query1 +"group by sms.providers.name  ";
 ...
 // rs1 = null;
 // int Zain = 1;
  //rs1 = con1.createStatement().executeQuery("select sms.providers.name, sms.mo_cdrs.shortcode , sum(mo_cdrs.length) as total , sum(mo_cdrs.length)/(SELECT sum(mo_cdrs.length) FROM sms.mo_cdrs JOIN sms.providers on mo_cdrs.Provider = providers.gateway  where DateIn between '"+sdate_formated+"' and '"+edate_formated+"' and sms.mo_cdrs.shortcode in ("+listOfShortCodes+")  )*100 as precent from sms.mo_cdrs JOIN sms.providers on mo_cdrs.Provider = providers.gateway where DateIn between '"+sdate_formated+"' and '"+edate_formated+"' and sms.mo_cdrs.shortcode in ("+listOfShortCodes+")and (sms.providers.name = 'SudaTel') group by mo_cdrs.shortcode asc ");
  //while(rs1.next()){
  	 // Zain = rs1.getInt(1); 
 // }
  rs1 = null;
  ...
  int SudaTel = 1;
 rs = con.createStatement().executeQuery("select sms.providers.name, sms.mo_cdrs.shortcode , sum(mo_cdrs.length) as total , sum(mo_cdrs.length)/(SELECT sum(mo_cdrs.length) FROM sms.mo_cdrs JOIN sms.providers on mo_cdrs.Provider = providers.gateway  where DateIn between '"+sdate_formated+"' and '"+edate_formated+"' and sms.mo_cdrs.shortcode in ("+listOfShortCodes+")  )*100 as precent from sms.mo_cdrs JOIN sms.providers on mo_cdrs.Provider = providers.gateway where DateIn between '"+sdate_formated+"' and '"+edate_formated+"' and sms.mo_cdrs.shortcode in ("+listOfShortCodes+")and (sms.providers.name = 'SudaTel') group by mo_cdrs.shortcode asc ");
  while(rs1.next()){
