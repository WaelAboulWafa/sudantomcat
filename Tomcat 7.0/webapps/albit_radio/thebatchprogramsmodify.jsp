...
for (int i =0;i< count ;i ++)
{
  if(ischeck[i] != null)
  {
      try{
    	  ...
    	  String query="";
          query= "update  sms.albit_pgms set pgm_name='" + (new String(pgmname[i].getBytes("Cp1252"),"Cp1256")) + "',airing_weekday='" + pgmairingday[i] + "',airing_from='" + pgmairingstart[i] + "', airing_to='" + pgmairingend[i] + "' , replay_weekday='" + pgmreplayday[i] + "',replay_from = '" + pgmreplaystart[i] + "' , replay_to= '" + pgmreplayend[i] + "' , username = '" + (new String(theusr[i].getBytes("Cp1252"),"Cp1256")) + "' , password= '" + (new String(thepass[i].getBytes("Cp1252"),"Cp1256")) + "'  where id= " +id[i] ;
