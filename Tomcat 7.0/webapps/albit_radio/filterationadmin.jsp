...
else
{
  ...
  try
  {
   rs = con.createStatement().executeQuery("SELECT userID,phone from fitservice.albit_pgm_Users where username=N'" + username.replaceAll("'", "''") + "' and password=N'" + password.replaceAll("'", "''") + "'");             
   userid = 0;
...
//   rs = con.createStatement().executeQuery("SELECT  CONVERT(char(10), lastlogintime, 101), convert(varchar,lastlogintime, 8)  from fitservice.albit_pgm_Users where userID=" + userid);             
   rs = con.createStatement().executeQuery("SELECT  lastlogintime,  lastlogintime  from fitservice.albit_pgm_Users where userID=" + userid);                
...
//-----------------------------------
/*String query1 = "SELECT count(*) from fitservice.albit_pgm_smss where flag=0";
String query2 = "SELECT ID from fitservice.albit_pgm_smss where flag=0 order by ID";
String query3 = "select fitservice.albit_pgm_smss.ID,fitservice.albit_pgm_smss.intime, messages.messages.Recipient,messages.messages.Sender, messages.messages.bodyformat,messages.messages.body from  fitservice.albit_pgm_smss INNER JOIN messages.messages  ON fitservice.albit_pgm_smss.ID = messages.messages.id and fitservice.albit_pgm_smss.flag=0 order by fitservice.albit_pgm_smss.ID  asc";
*/
  ...
  <tr>
    ...
    <td width='24%' align='center'>
    	<table border='0' cellpadding='0' cellspacing='0' style='border-collapse: collapse' bordercolor='#A36103' width='100%'>
 		...
 		<form name='theform' action='filteration.jsp' method='post'>
