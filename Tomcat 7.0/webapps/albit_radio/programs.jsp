...
else
{
  ...
  String generalpass="";
  rs = con.createStatement().executeQuery("SELECT thepassword from sms.albit_superusers where theuser='general'");             
  while(rs.next()){generalpass = rs.getString(1); }rs.close(); rs = null;

		   
  String adminpass="";
  rs = con.createStatement().executeQuery("SELECT thepassword from sms.albit_superusers where theuser='admin'");             
...
//try{pgmstatus = request.getParameter("pgmstatus").trim();}catch(Exception e){pgmstatus = "0";}
 rs = con.createStatement().executeQuery("SELECT status FROM sms.albit_operations");            
...
<% 
   ...
   rs = con.createStatement().executeQuery("SELECT id,pgm_name,airing_weekday,airing_from,airing_to,replay_weekday,replay_from,replay_to,username,password FROM sms.albit_pgms order by id");  
