...
<%
	...
	if (username == null || password == null) //user not logged in
	{
		...
		int phoneallowed = 0;
		try {
			rs = con.createStatement().executeQuery("SELECT userID,phone,programid from fitservice.albit_pgm_Users where username=N'"
							+ username.replaceAll("'", "''")
							+ "' and password=N'"
							+ password.replaceAll("'", "''") + "'");
			userid = 0;
		...
		try {
			rs = con.createStatement().executeQuery(
					"SELECT count(*) from fitservice.albit_pgm_UsersAuthorities where userID="
							+ userid);
			while (rs.next()) {
		...
		try {
			rs = con.createStatement()
					.executeQuery(
							"SELECT page, allowed from fitservice.albit_pgm_UsersAuthorities where userID="
									+ userid);
			while (rs.next()) {
		...
		try {
			...
			rs = con.createStatement()
					.executeQuery(
							"SELECT  lastlogintime,  lastlogintime  from fitservice.albit_pgm_Users where userID="
									+ userid);
			while (rs.next()) {
		...
		if (allowedaccess == false) {
			...
			/*String query3 = "select fitservice.albit_pgm_smss.ID, (ID)
							   	fitservice.albit_pgm_smss.intime, (datein)
								...
								from  
								fitservice.albit_pgm_smss INNER JOIN messages.messages 
								ON fitservice.albit_pgm_smss.ID = messages.messages.id 
								and fitservice.albit_pgm_smss.flag=0  order by 
								fitservice.albit_pgm_smss.ID  asc";*/
...
<%
 ...
 <td align='center'><b><font color=#000099 size=5><%=txt   %></font></b></td>
