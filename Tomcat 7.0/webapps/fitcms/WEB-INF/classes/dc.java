...
{ 
    ...
    public void doGet(HttpServletRequest  request,HttpServletResponse response)throws IOException, ServletException
    {
     ...
     try{ 
      ...
      ResultSet rs = con.createStatement().executeQuery("SELECT     id, scode FROM         ComboPush where id='"+ id +"' and scode='"+ scode +"' and downloaded ='0' ORDER BY id");
       ...
       int thecontentcode = 0;
        rs = con.createStatement().executeQuery("SELECT     code FROM         ComboPush where id='"+ id +"' and scode='"+ scode +"' and downloaded ='0' ORDER BY id");
...
try{
       ...
       try
       {
              f = new FileInputStream(new File("C:/contents/" + FileName.trim()));
       }catch (Exception e)
                               {
                               	try
                               	   {
       ...
       response.setContentLength((int)(new File("C:/contents/" + FileName.trim())).length());
       response.addHeader("Content-Disposition","attachment;filename=" + downloadedname );
     
       int b = 0;
       while( (b= f.read()) != -1){out.write(b);}
       f.close();f=null;
       con.createStatement().execute( "UPDATE  ComboPush   SET downloaded = downloaded + 1 where ID= " + id.trim() );
       out.flush();            
       out.close();
