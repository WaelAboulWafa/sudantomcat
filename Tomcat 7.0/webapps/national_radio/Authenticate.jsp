...
response.sendRedirect("login.jsp");
  ...
  if(  username.equalsIgnoreCase("admin") || username.equalsIgnoreCase("general") )
  {
		...
		if( username.equalsIgnoreCase("admin"))
		{
		  ...
		  try
		  {
		   ...
		   int validadmin=0;
		   rs = con.createStatement().executeQuery("SELECT count(*) from sms.nradio_superusers where theuser='admin' and thepassword=N'" + password.replaceAll("'", "''") + "'");             
		...
		if( username.equalsIgnoreCase("general"))
		{
		  ...
		  try
		  {
		   ...
		   int validgeneral=0;
		   rs = con.createStatement().executeQuery("SELECT count(*) from sms.nradio_superusers where theuser='general' and thepassword=N'" + password.replaceAll("'", "''") + "'");             
  ...
  else
  {
		  ...
		  try
		  {
		   ...
		   rs = con.createStatement().executeQuery("SELECT id from sms.nradio_pgms where username=N'" + username.replaceAll("'", "''") + "' and password=N'" + password.replaceAll("'", "''") + "'");
