...
else
{
  ...
  try
  {
   rs = con.createStatement().executeQuery("SELECT userID,phone from fitservice.nradio_pgm_Users where username=N'" + username.replaceAll("'", "''") + "' and password=N'" + password.replaceAll("'", "''") + "'");             
   userid = 0;
...
//   rs = con.createStatement().executeQuery("SELECT  CONVERT(char(10), lastlogintime, 101), convert(varchar,lastlogintime, 8)  from fitservice.nradio_pgm_Users where userID=" + userid);             
   rs = con.createStatement().executeQuery("SELECT  lastlogintime,  lastlogintime  from fitservice.nradio_pgm_Users where userID=" + userid);                
...
//-----------------------------------
/*String query1 = "SELECT count(*) from fitservice.nradio_pgm_smss where flag=0";
String query2 = "SELECT ID from fitservice.nradio_pgm_smss where flag=0 order by ID";
String query3 = "select fitservice.nradio_pgm_smss.ID,fitservice.nradio_pgm_smss.intime, messages.messages.Recipient,messages.messages.Sender, messages.messages.bodyformat,messages.messages.body from  fitservice.nradio_pgm_smss INNER JOIN messages.messages  ON fitservice.nradio_pgm_smss.ID = messages.messages.id and fitservice.nradio_pgm_smss.flag=0 order by fitservice.nradio_pgm_smss.ID  asc";
*/
