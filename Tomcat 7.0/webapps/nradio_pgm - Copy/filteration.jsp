...
<%
	...
	if (username == null || password == null) //user not logged in
	{
		...
		int phoneallowed = 0;
		try {
			rs = con.createStatement().executeQuery("SELECT userID,phone,programid from fitservice.nradio_pgm_Users where username=N'"
							+ username.replaceAll("'", "''")
							+ "' and password=N'"
							+ password.replaceAll("'", "''") + "'");
			userid = 0;
		...
		if (allowedaccess == false) {
			...
			/*String query3 = "select fitservice.nradio_pgm_smss.ID, (ID)
							   	fitservice.nradio_pgm_smss.intime, (datein)
								...
								from  
								fitservice.nradio_pgm_smss INNER JOIN messages.messages 
								ON fitservice.nradio_pgm_smss.ID = messages.messages.id 
								and fitservice.nradio_pgm_smss.flag=0  order by 
								fitservice.nradio_pgm_smss.ID  asc";*/
